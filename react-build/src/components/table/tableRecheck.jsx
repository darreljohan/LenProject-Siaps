...
const TableRecheck = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:3000/table/mergeSpreadsheetTable?action=getTable&destinationSheet=Len&tableName=len_Assets').then(
            (response) => {
    ...
    return (
        ...
        <table {...getTableProps()} className="text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            {rows.map((row, index) => {
              ...
              return (
                ...
                return (
                    <tr {...row.getRowProps()} className="bg-green-400 border-b dark:bg-gray-900 dark:border-gray-700">
                      {row.cells.map((cell) => {
                        return <td {...cell.getCellProps()} className="px-6 py-4">{cell.render('Cell')}</td>;
                      })}
                    </tr>
